...
class FMSQuery:
    ...
    def _get_grouped_functional_tests(self) -> None:
        ...
        self.grouped_functional_tests = grouped_functional_tests
    ...
    def closed_loop_analysis(self, show_table: bool = True) -> None:
        ...
        table_mapping = [
            {"type": "Time to open", "clarification": f"TV power ON to 0.1 mg/s {gas_type} total flow rate"},
            {"type": "Time to regulated mass flow", "clarification": f"Time to regulated setpoint of 1.0 barA starting at 0.1 mg/s {gas_type} total flow rate"},
            {"type": "Time to regulated mass flow", "clarification": "Time to regulated setpoint of 1.0 barA to 1.625 barA"},
            {"type": "Time to regulated mass flow", "clarification": "Time to regulated setpoint of 1.625 barA to 2.25 barA"},
            {"type": "Time to regulated mass flow", "clarification": "Time to regulated setpoint of 2.25 barA to 1.625 barA"},
            {"type": "Time to regulated mass flow", "clarification": "Time to regulated setpoint of 1.625 barA to 1.0 barA"},
            {"type": "Time to close", "clarification": "TV power OFF to LPT pressure of 0.2 barA*"}
        ]
        closed_loop_data = {
        ...
        for test_type, temp_groups in grouped_functional_tests.items():
            ...
            for temp_type, tests in temp_groups.items():
                ...
                for test in tests:
                    ...
                    pressure_label = "low" if inlet_pressure == 10 else "high"
        ...
        order = ["room_pre_vibration", FunctionalTestType.HOT, FunctionalTestType.COLD, FunctionalTestType.ROOM]
        closed_loop_data = [closed_loop_data[k] for k in order]
        if show_table:
            display(output)
            df = pd.DataFrame(test_info)
            df = df.fillna("N/A")
    ...
    def part_investigation_query(self, part_type: str) -> None:
        ...
        if part_type == "Thermal Valve":
            tv_query = TVQuery(session=self.session, fms_entry=self.fms_entry, local=self.local)
            tv_query.tv_query_field()

        elif part_type == "Manifold":
            manifold_query = ManifoldQuery(session=self.session, fms_entry=self.fms_entry, local=self.local)
