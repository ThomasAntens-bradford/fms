...
class ManifoldQuery:
    ...
    def __init__(self, session: "Session", fms_entry: FMSMain = None, min_radius: float = 0.22, max_radius: float = 0.25, ref_thickness: float = 0.25, 
    ...
    def fr_status(self, fr_id: str = None) -> None:
        ...
        tools: list[dict] = fr_entry.tools
        tools_for_df = [{key.replace("_", " ").title(): value for key, value in i.items() if key != 'type'} for i in tools]

        tools_df = pd.DataFrame(tools_for_df)
    ...
    def fr_correction_simulation(self) -> None:
        ...
        def simulate_and_plot(anode_batch, cathode_batch,
            ...
            anode_relations = self.fr_flow_analysis(certification = 'all', fr_entry = None, fr_type = 'Anode', plot = False, correction = True)
            cathode_relations = self.fr_flow_analysis(certification = 'all', fr_entry = None, fr_type = 'Cathode', plot = False, correction = True)
    ...
    def fr_flow_analysis(self, certification: str | list[str], fr_entry: AnodeFR | CathodeFR, fr_type: str,
        ...
        for fr in filtered_frs:
            ...
            if fr.orifice_diameter and fr.flow_rates and fr.pressures:
                ...
                for i, p_val in enumerate(pressures_of_interest):
                    ...
                    if error:
                        fs_error = self.anode_fs_error if self.anode else self.cathode_fs_error
                        err = max(fs_error, self.reading_error * flow)
                        error_lists[i].append(err)
                    else:
        ...
        reference_values = [self.anode_target_15 if self.anode else self.cathode_target_15,
                            self.anode_target_24 if self.anode else self.cathode_target_24]
        reference_orifice = self.anode_reference_orifice if self.anode else self.cathode_reference_orifice
    ...
    def select_ratio(self) -> None:
        ...
        def on_ratio_change(change):
            ratio = change['new'] if change['new'] else 13.0
            with output:
                output.clear_output()
                self.match_flow_restrictors(ratio)
        ...
        with output:
            output.clear_output()
            self.match_flow_restrictors(13.0)
    ...
    def match_flow_restrictors(self, ratio: float = 13, tolerance: float = 0.5,
        ...
        styled_df_all = df_all.style.hide(axis = 'index')
        display_df_in_chunks(styled_df_all)
        ...
        format_dict = {
            col: "{:.2f}" for col in df_max.columns if col.startswith("Ratio")
        }
