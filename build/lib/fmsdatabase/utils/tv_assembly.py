...
class TVAssembly:
    ...
    def __init__(self, session: "Session", fms: "FMSDataStructure", word_template_path: str = r"TVAssemblyProcedure\tv_assembly_procedure.docx",
        ...
        if operator_check:
            ...
            try:
                ...
                self.operator = "NRN"
            save_to_json(self.operator, "operator")
        self.adhesive_logs = []
    ...
    def get_steps(self, tv_id: int) -> dict:
        ...
        try:
            ...
            if entry:
                return entry.steps
            return self.fms.load_procedure('tv_assembly_steps')
        except Exception as e:
    ...
    def on_tv_change(self, change: dict) -> None:
        ...
        Callback function to handle changes in the selected TV ID from the dropdown.
        Args:
            change (dict): The change dictionary from the widget observer.
        """
        new_tv_id = change['new']
        self.header_output.clear_output()
        self.tv_id = new_tv_id
        self.tv_id_widget.value = self.tv_id
        self.steps: dict = self.get_steps(self.tv_id)
        self.start_date_obj = datetime.strptime(self.steps.get('start_date'), "%B %d, %Y").date() if self.steps.get('start_date') else None

        if self.steps:
            self.resistance_goal = self.steps.get('resistance_goal', 150)
            max_step = max(int(k) for k in self.steps.keys() if k.isdigit())
            current_step = next((step for step, data in self.steps.items() if step.isdigit() and data.get("performed", False) == False), max_step)
            if current_step and current_step != "32":
                with self.output:
                    self.output.clear_output()
                    print(f"Resuming from step {current_step} for TV ID {self.steps.get('tv_id', None)}.")
                self.start_date = self.steps.get('start_date', None)
                self.procedure_loop(step_number=int(current_step))
            else:
                self.start_assembly()
        else:
            with self.header_output:
                print(f"No draft found for TV ID {self.tv_id}.")
    ...
    def start_assembly(self) -> None:
        ...
        resistance_goal_widget = widgets.BoundedIntText(
            value=self.steps.get('resistance_goal', 150),
            min=0,
            max=1000,
        ...
        start_date_widget = widgets.DatePicker(
            **self.field("Start Date:"),
            value=datetime.strptime(self.steps['start_date'], "%B %d, %Y").date() if self.steps.get('start_date') else datetime.now().date(),
        )
        
        def on_field_change(change: dict) -> None:
            self.steps['tv_id'] = self.tv_id_widget.value
            self.steps['resistance_goal'] = resistance_goal_widget.value
            self.steps['start_date'] = start_date_widget.value.strftime("%B %d, %Y") if start_date_widget.value else None

        self.tv_id_widget.observe(on_field_change, names='value')
        resistance_goal_widget.observe(on_field_change, names='value')
        start_date_widget.observe(on_field_change, names='value')

        def on_button_click(b) -> None:
            self.output.clear_output()
            self.steps = {}
            self.tv_id = self.tv_id_widget.value
            self.resistance_goal = resistance_goal_widget.value
            self.start_date = start_date_widget.value.strftime("%B %d, %Y") if start_date_widget.value else None
            self.start_date_obj = start_date_widget.value
            self.steps = self.get_steps(self.tv_id)
            current_options = [self.all_tvs_field.options[i][1] for i in range(len(self.all_tvs_field.options))]
            if not self.tv_id in current_options:
                self.all_tvs_field.options = list(self.all_tvs_field.options) + [(f"TV {self.tv_id}", self.tv_id)]
        ...
        title = widgets.HTML(value="<h2>TV Coil Assembly Procedure</h2>")

        form = widgets.VBox(
            [title, widgets.VBox([self.tv_id_widget, resistance_goal_widget, start_date_widget]), start_button, self.output],
    ...
    def assembly_procedure(self) -> None:
        ...
        if not self.tv_id or (self.tv_id and not str(self.tv_id).isdigit()):
            with self.header_output:
                print("Please select a TV ID.")
            return
        ...
        if not self.resistance_goal:
            with self.header_output:
                print("Please enter a valid resistance goal.")
            return
        ...
        assembly_check = self.session.query(CoilAssembly).filter(CoilAssembly.tv_id==self.tv_id, CoilAssembly.context.isnot(None)).first()
        if assembly_check:
            self.steps: dict[str, Any] = assembly_check.steps
            self.context: dict[str, Any] = assembly_check.context
            ...
            self.start_date_obj = datetime.strptime(self.start_date, "%B %d, %Y").date() if self.start_date else None
            self.get_certifications()
            return
        
        max_step = max(int(k) for k in self.steps.keys() if k.isdigit())
        start_step = next((step for step, data in self.steps.items() if step.isdigit() and data.get("performed", False) == False), str(max_step))
        ...
        if start_step != "32":
            self.output.clear_output()
            with self.output:
                print(f"Resuming from step {start_step} for TV ID {self.steps.get('tv_id', None)}.")
